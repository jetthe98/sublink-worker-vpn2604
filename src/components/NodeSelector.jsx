    ...
    <div
      x-data="{
        ...
        startEditProxyName(proxyName, event) {
            ...
            this.tempEditName = this.getProxyDisplayName(proxyName);
            this.editingProxyName = proxyName;
        },

        saveProxyName(proxyName, event) {
            event.stopPropagation();
            if (event.key === 'Escape') {
                this.cancelEditName();
                return;
            ...
            if (event.key === 'Enter' || event.type === 'blur') {
                if (this.tempEditName.trim() !== '') {
                    this.setCustomProxyName(proxyName, this.tempEditName);
                }
                ...
                <div class="flex items-center gap-2">
                  <template x-if="editingProxyName !== proxy.name">
                    <span
                      ...
                      x-text="getProxyDisplayName(proxy.name)"
                  ...
                  <template x-if="hasCustomName(proxy.name)">
                    <span
              ...
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-4">
                <div class="flex items-center justify-between mb-3">
                  <input
                    ...
                    x-model="group.name"
                    placeholder="分组名称"
                    class="flex-1 px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent dark:bg-gray-700 dark:text-white"
                  />
                ...
                <div class="flex flex-wrap gap-2">
                  <template x-for="proxyName in group.proxies" x-bind:key="proxyName">
                    <span class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded">
                      <span x-text="getProxyDisplayName(proxyName)" class="max-w-24 truncate"></span>
